...
- Providers: doubao, elevenlabs, minimax, inworld, fishaudio, speechify, openai, gemini, mistral, cartesia, and kokoro. All are opt-in; use only choices listed as configured in the capabilities prompt, and never mix voice catalogs.
- Kokoro: English only, Apple Silicon Mac CPU. Download in Settings → Voice / TTS → Kokoro Local first; never download during generation or fall back to cloud. Saved Settings voice/speed are confirmed choices: use their concrete values without asking again. Otherwise ask for Heart af_heart / Michael am_michael (US), Emma bf_emma / George bm_george (UK); audition the fixed sample in Settings. Only text/voiceId/speed/name are supported. Long narration is bounded internally without truncation and returns one completed WAV.
- Curated choices exist for Doubao, ElevenLabs, MiniMax, and Kokoro. Bundled samples exist only where references/voices.md lists one; Kokoro audition is generated locally, not a sample URL. For every other provider, do not invent a preset or sample URL; require a concrete voiceId from the user/provider account.
